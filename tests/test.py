...
def runInstances(ins):
    for in_ in ins:
        cmd = "docker run -d -p %d:13800 " \
              ...
              "-e REPL_FACTOR=%d " \
              "kvs:4.0" % \
              (in_["host_port"], in_["subnet"], in_["ip_address"], in_[
               "name"], in_["ip_address"], in_["view"], in_["repl_factor"])
...
class TestHW3(unittest.TestCase):
    ...
    def key_count_helper(self, response):
        ...
        self.assertEqual(response["message"],
                         "Key count retrieved successfully")
    ...
    def get_shards_helper(self, response, shard_count):
        self.assertEqual(response["status_code"], 200)
        self.assertEqual(response["message"],
                         "Shard membership retrieved successfully")
    ...
    def gossip_helper(self, shard_count, nodes, ins):
        client = Client(causal_context_flag=False,
        ...
        for i in range(keys):
            ...
            address, port = ins[i % node_count]["address"], ins[i %
                                                                node_count]["host_port"]
            ...
            shard_id = self.check_shard_id_by_address(
                response, shards, address)
            ...
            self.assertEqual(shard_id, self.check_shard_id_by_address(
                response, shards, address))
    ...
    def view_change_helper(self, old_shard_count, old_nodes, old_ins, new_shard_count, new_repl_factor, new_nodes, new_view, new_ins):
        ...
        for i in range(keys):
            ...
            address, port = old_ins[i % old_node_count]["address"], old_ins[i %
                                                                            old_node_count]["host_port"]
        ...
        for i in range(keys):
            key = "test_view_change_%d" % i
            value = "I do care %d" % i
            address, port = new_ins[i % new_node_count]["address"], new_ins[i %
                                                                            new_node_count]["host_port"]
            # get
            response = client.getKey(key, port)
            expected = getResponse_Success.copy()
            expected["value"] = value
    ...
    def test_gossip_1(self):
        ...
        view = ",".join(nodes)
        ins = [
            {"subnet": "kv_subnet", "host_port": 13800, "ip_address": "10.10.0.2",
                "address": "10.10.0.2:13800", "name": "node1", "view": view, "repl_factor": repl_factor},
            {"subnet": "kv_subnet", "host_port": 13801, "ip_address": "10.10.0.3",
                "address": "10.10.0.3:13800", "name": "node2", "view": view, "repl_factor": repl_factor},
        ]
    ...
    def test_gossip_2(self):
        ...
        ins = [
            ...
            {"subnet": "kv_subnet", "host_port": 13801, "ip_address": "10.10.0.3",
                "address": "10.10.0.3:13800", "name": "node2", "view": view, "repl_factor": repl_factor},
            {"subnet": "kv_subnet", "host_port": 13802, "ip_address": "10.10.0.4",
                "address": "10.10.0.4:13800", "name": "node3", "view": view, "repl_factor": repl_factor},
            {"subnet": "kv_subnet", "host_port": 13803, "ip_address": "10.10.0.5",
                "address": "10.10.0.5:13800", "name": "node4", "view": view, "repl_factor": repl_factor},
        ]
    ...
    def test_view_change_1(self):
        ...
        old_ins = [
            {"subnet": "kv_subnet", "host_port": 13800, "ip_address": "10.10.0.2",
                "address": "10.10.0.2:13800", "name": "node1", "view": old_view, "repl_factor": old_repl_factor},
        ]
        new_ins = [
            {"subnet": "kv_subnet", "host_port": 13800, "ip_address": "10.10.0.2",
                "address": "10.10.0.2:13800", "name": "node1", "view": old_view, "repl_factor": old_repl_factor},
            {"subnet": "kv_subnet", "host_port": 13801, "ip_address": "10.10.0.3",
                "address": "10.10.0.3:13800", "name": "node2", "view": old_view, "repl_factor": old_repl_factor},
            {"subnet": "kv_subnet", "host_port": 13802, "ip_address": "10.10.0.4",
                "address": "10.10.0.4:13800", "name": "node3", "view": old_view, "repl_factor": old_repl_factor},
            {"subnet": "kv_subnet", "host_port": 13803, "ip_address": "10.10.0.5",
                "address": "10.10.0.5:13800", "name": "node4", "view": old_view, "repl_factor": old_repl_factor},
        ]

        new_shard_count, new_repl_factor, new_nodes = 2, 2, [
            "10.10.0.2:13800", "10.10.0.3:13800", "10.10.0.4:13800", "10.10.0.5:13800"]
        new_view = ",".join(new_nodes)

        self.view_change_helper(old_shard_count, old_nodes, old_ins,
                                new_shard_count, new_repl_factor, new_nodes, new_view, new_ins)

    def test_view_change_2(self):
        old_shard_count, old_repl_factor, old_nodes = 1, 2, [
    ...
    def test_view_change_3(self):
        ...
        new_shard_count, new_repl_factor, new_nodes = 1, 2, [
            "10.10.0.4:13800", "10.10.0.5:13800"]
        new_view = ",".join(new_nodes)
    ...
    def test_network_partition_1(self):
        ...
        shard_count, repl_factor, nodes = 1, 2, [
            "10.10.0.2:13800", "10.11.0.2:13800"]
        view = ",".join(nodes)
        ins = [
            {"subnet": "kv_subnet", "host_port": 13800, "ip_address": "10.10.0.2",
                "address": "10.10.0.2:13800", "name": "node1", "view": view, "repl_factor": repl_factor},
            {"subnet": "kv_subnet_partition", "host_port": 13801, "ip_address": "10.11.0.2",
                "address": "10.11.0.2:13800", "name": "node2", "view": view, "repl_factor": repl_factor},
        ]
        ...
        self.assertTrue(response1["value"] ==
                        "node1" or response1["value"] == "node2")
